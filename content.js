...
class YouTubeSummarizer {
  ...
  async generateCurrentSummary(forceRegenerate = false) {
    ...
    // Check if we already have this summary (unless forcing regeneration)
    ...
    if (existingSummary && !forceRegenerate) {
      ...
      console.log('💡 To regenerate, call: window.youtubeSummarizer.generateCurrentSummary(true)');
      this.updateSummaryDisplay();
      return;
    }
    
    if (existingSummary && forceRegenerate) {
      console.log('🔄 Regenerating summary (cache exists but forcing new generation)');
    }
  ...
  // Helper function to force regenerate current summary
  async forceRegenerateSummary() {
    console.log('🔄 Force regenerating current summary...');
    await this.generateCurrentSummary(true);
  }
  ...
  debugCacheStatus() {
    ...
    if (hasCached) {
      ...
      console.log('  1. Force regenerate: window.youtubeSummarizer.forceRegenerateSummary()');
      console.log('  2. Clear cache: window.youtubeSummarizer.clearSummaryCache()');
      console.log('  3. Generate with force: window.youtubeSummarizer.generateCurrentSummary(true)');
    } else {
        ...
        summaryContent.innerHTML = `
          <div class="summarizer-error">
            ...
            <div class="error-actions">
              <button class="regenerate-summary-btn" onclick="(function(){if(window.youtubeSummarizer && window.youtubeSummarizer.forceRegenerateSummary) {window.youtubeSummarizer.forceRegenerateSummary();} else {console.warn('YouTubeSummarizer not loaded, please refresh the page');}})();">
                🔄 Try Again
              </button>
            </div>
          </div>
...
function initializeSummarizer() {
  ...
  try {
    ...
    window.youtubeSummarizer.forceRegenerate = function() {
      this.forceRegenerateSummary();
    };
    ...
    window.youtubeSummarizer.quickFix = function() {
      ...
      console.log('  forceRegenerate() - Force regenerate current summary');
      console.log('  debugCache() - Show detailed cache information');
      console.log('  generateCurrentSummary(true) - Generate with force flag');
    };
