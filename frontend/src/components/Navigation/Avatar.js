...
const Avatar = ({ 
  ...
  // Get source from user object or props
  const avatarSrc = src || user?.picture;
  const avatarName = name || user?.name;
  const avatarAlt = alt || `${user?.name || 'User'} avatar`;
  ...
  useEffect(() => {
    ...
    console.log('🖼️ Avatar: Starting image load for:', avatarSrc?.substring(0, 50) + '...');
    setImageState('loading');
    setImageSrc(null);
    
    // Start loading attempt
    attemptImageLoad(avatarSrc);
      ...
      default:
        return (
          <FallbackAvatar size={size} border={border}>
            {getInitials(avatarName || avatarAlt)}
          </FallbackAvatar>
        );
    }
  };
